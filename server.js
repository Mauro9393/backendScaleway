...
app.post("/api/:service", upload.none(), async (req, res) => {
    ...
    try {
        ...
        // --- NEW: Azure TTS streaming via GET (progressive download) ---
        else if (service === "azureTTS-Scaleway-stream") {
            const {
                text,
                selectedLanguage,
                selectedVoice,
                ...
                pitch,
                volume
            } = req.query; // <— querystring, non body
            ...
            // Riusa la tua buildSSML()
            const ssml = buildSSML({ text, voice, style, styleDegree, rate, pitch, volume });

            try {
                // QUI: chiediamo ad Azure l’audio in streaming
                const ttsResp = await axios.post(
                    ...
                    ssml,
                    {
                        headers: {
                            "Ocp-Apim-Subscription-Key": apiKey,
                            "Content-Type": "application/ssml+xml",
                            // bitrate più basso = meno latenza di download
                            "X-Microsoft-OutputFormat": "audio-16khz-24kbitrate-mono-mp3"
                        },
                ...
                res.setHeader("Content-Type", "audio/mpeg");
                res.setHeader("Transfer-Encoding", "chunked"); // opzionale, spesso messo automaticamente
                ttsResp.data.pipe(res); // <— STREAM DIRETTO VERSO IL CLIENT

            } catch (err) {
                const details = err.response?.data?.toString?.() || err.response?.data || err.message;
                console.error("Azure Speech TTS streaming error:", details);
                return res.status(err.response?.status || 500)
                    .json({ error: "Azure Speech TTS streaming failed", details });
            }
        }
